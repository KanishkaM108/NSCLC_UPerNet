...
FOLD_EXPERIMENTS = [
    "upernet_swin_tiny_hybrid_fold0",
    "upernet_swin_tiny_hybrid_fold1",
    "upernet_swin_tiny_hybrid_fold2",
    "upernet_swin_tiny_hybrid_fold3",
    "upernet_swin_tiny_hybrid_fold4",
]
...
def save_qualitative(image, target, prediction, class_names, path):
    ...
    if scale < 1.0:
        ...
        pred_rgb = np.asarray(
            Image.fromarray(pred_rgb).resize(new_size, Image.Resampling.NEAREST)
        )
        overlay = np.asarray(
            Image.fromarray(overlay).resize(new_size, Image.Resampling.LANCZOS)
        )
    fig, axes = plt.subplots(1, 4, figsize=(18, 5))
    panels = [image, target_rgb, pred_rgb, overlay]
    titles = ["H&E", "Ground truth", "Five-fold ensemble", "Overlay"]
...
def save_metric_figures(confusion, metrics, class_names, output_dir):
    ...
    axis.set_title("Five-fold ensemble: row-normalized test confusion matrix")
    ...
    axis.barh(np.asarray(class_names)[order], np.asarray(values)[order], color="tab:purple")
    axis.set_xlim(0, 1)
    axis.set_xlabel("F1 / Dice")
    axis.set_title("Five-fold ensemble per-class holdout performance")
...
def build_group_lookup(label_spec):
    ...
    return grouped_names, lookup


def load_ensemble(cfg, label_spec, device):
    checkpoint_paths = [Path(cfg["output_root"]) / name / "best.pt" for name in FOLD_EXPERIMENTS]
    missing = [str(path) for path in checkpoint_paths if not path.exists()]
    if missing:
        raise SystemExit("Missing fold checkpoint(s):\n  " + "\n  ".join(missing))

    models = []
    checkpoint_epochs = []
    checkpoint_scores = []
    for fold_index, path in enumerate(checkpoint_paths):
        print(f"Loading fold {fold_index}: {path}")
        model = build_upernet(label_spec, cfg["pretrained_checkpoint"])
        checkpoint = torch.load(path, map_location="cpu", weights_only=False)
        model.load_state_dict(checkpoint["model"])
        model.eval()
        models.append(model)
        checkpoint_epochs.append(int(checkpoint["epoch"]))
        checkpoint_scores.append(
            float(checkpoint.get("best_val_macro_f1", float("nan")))
        )
        del checkpoint
    ...
    try:
        ...
        print("GPU cannot hold all five models together; switching to automatic CPU offload mode.")
        for model in models:
            model.to("cpu")
        torch.cuda.empty_cache()
        execution_mode = "per_roi_cpu_offload"

    return models, checkpoint_paths, checkpoint_epochs, checkpoint_scores, execution_mode


def infer_batches_with_model(
    model,
    image_pad,
    coordinates,
    tile_size,
...
def main() -> None:
    ...
    result_dir = (
        ...
        / "upernet_swin_tiny_5fold_ensemble"
    ...
    models, checkpoint_paths, checkpoint_epochs, checkpoint_scores, execution_mode = (
        load_ensemble(cfg, label_spec, device)
    )
    print(f"Ensemble execution mode: {execution_mode}")

    num_classes = label_spec["num_classes"]
    confusion = np.zeros((num_classes, num_classes), dtype=np.int64)
    grouped_names, group_lookup = build_group_lookup(label_spec)
    ...
    with torch.inference_mode():
        ...
        for row_index, row in enumerate(progress):
            ...
            probability_sum = np.zeros((num_classes, padded_h, padded_w), dtype=np.float32)
            ...
            if execution_mode == "all_models_gpu_resident":
                ...
                for fold_index, model in enumerate(models):
                    progress.set_postfix_str(f"model {fold_index + 1}/5")
                    model.to(device)
                    infer_batches_with_model(
                        model,
                        image_pad,
            ...
            probability_sum /= np.maximum(
                weight_sum[None, :, :] * len(models), 1e-8
            )
    ...
    metrics["ensemble_method"] = "mean softmax probability"
    ...
    if fold0_metrics_path.exists():
        with open(fold0_metrics_path, "r", encoding="utf-8") as handle:
            fold0_metrics = json.load(handle)
        metrics["change_vs_fold0_overlap"] = {
            "pixel_accuracy": metrics["pixel_accuracy"] - fold0_metrics["pixel_accuracy"],
            "macro_f1_dice": metrics["macro_f1_dice"] - fold0_metrics["macro_f1_dice"],
            "mean_iou": metrics["mean_iou"] - fold0_metrics["mean_iou"],
            "grouped_f1_dice": metrics["til_grouped"]["macro_f1_dice"]
            - fold0_metrics["til_grouped"]["macro_f1_dice"],
        }
    ...
    print("=" * 72)
    print("FIVE-FOLD PROBABILITY-ENSEMBLE UNTOUCHED HOLDOUT RESULTS")
    print("=" * 72)
    print(f"Pixel accuracy : {metrics['pixel_accuracy']:.4f}")
    print(f"Macro F1/Dice  : {metrics['macro_f1_dice']:.4f}")
    print(f"Mean IoU       : {metrics['mean_iou']:.4f}")
    print(f"Grouped F1     : {metrics['til_grouped']['macro_f1_dice']:.4f}")
    if "change_vs_fold0_overlap" in metrics:
        change = metrics["change_vs_fold0_overlap"]
